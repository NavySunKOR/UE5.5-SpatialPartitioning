// Fill out your copyright notice in the Description page of Project Settings.

#pragma once

#include "CoreMinimal.h"
#include "SpatialPartitioning/SPBaseActor.h"
#include "SPQuadTreeActor.generated.h"

/**
 * 
 */

struct QuadTreeNodeElement
{
public:
	TWeakObjectPtr<UStaticMeshComponent> Component;
	FVector2D QTPosition;
};

class QuadTreeNode
{

public:
	QuadTreeNode(FBox2D InBound, int32 InDepth)
	{
		Bound = InBound;
		Depth = InDepth;
	}

	void InsertData(const QuadTreeNodeElement& InTreeElem, const int32& InMaximumDepth)
	{
		if (Bound.IsInside(InTreeElem.QTPosition) == false)
		{
			return;
		}

		if (Depth == InMaximumDepth)
		{
			Items.Add(InTreeElem);
			return;
		}

		if (bIsSubdivided == false)
		{
			Subdivide();
		}

		NW->InsertData(InTreeElem, InMaximumDepth); // Duplicate?
		NE->InsertData(InTreeElem, InMaximumDepth);
		SW->InsertData(InTreeElem, InMaximumDepth);
		SE->InsertData(InTreeElem, InMaximumDepth);
	}

	void GetNearestElements(TArray<QuadTreeNodeElement>& OutData, const FVector2D& InLookAtPos, const int32& InMaximumDepth)
	{
		if (Bound.IsInside(InLookAtPos) == false)
		{
			return;
		}
		if (Depth < InMaximumDepth)
		{
			if (bIsSubdivided)
			{
				NW->GetNearestElements(OutData, InLookAtPos, InMaximumDepth); // Duplicate?
				NE->GetNearestElements(OutData, InLookAtPos, InMaximumDepth);
				SW->GetNearestElements(OutData, InLookAtPos, InMaximumDepth);
				SE->GetNearestElements(OutData, InLookAtPos, InMaximumDepth);
			}
			return;
		}

		UE_LOG(LogTemp,Warning,TEXT("Adding nearest...."))

		for (const QuadTreeNodeElement& Item : Items)
		{
			OutData.Add(Item);
		}
	}

	void GetBoundaries(TArray<FBox2D>& OutIteration, const int32& InMaxDepth)
	{
		if (Depth == InMaxDepth)
		{
			OutIteration.Add(Bound);
			return;
		}

		if (bIsSubdivided)
		{
			NW->GetBoundaries(OutIteration, InMaxDepth);
			NE->GetBoundaries(OutIteration, InMaxDepth);
			SW->GetBoundaries(OutIteration, InMaxDepth);
			SE->GetBoundaries(OutIteration, InMaxDepth);
		}
	}

protected:
	void Subdivide()
	{
		//TODO: Calc Bound......

		const FVector2D NewExtent = Bound.GetExtent() / 2.f;
		const FVector2D Center = Bound.GetCenter();

		FVector2D NWCenter = Center;
		NWCenter.X -= NewExtent.X;
		NWCenter.Y += NewExtent.Y;

		const FVector2D NECenter = Center + NewExtent;

		const FVector2D SWCenter = Center - NewExtent;

		FVector2D SECenter = Center;
		SECenter.X += NewExtent.X;
		SECenter.Y -= NewExtent.Y;

		const FBox2D NWBox = FBox2D::BuildAABB(NWCenter,NewExtent);
		const FBox2D NEBox = FBox2D::BuildAABB(NECenter, NewExtent);
		const FBox2D SWBox = FBox2D::BuildAABB(SWCenter, NewExtent);
		const FBox2D SEBox = FBox2D::BuildAABB(SECenter, NewExtent);

		const int32 NextDepth = Depth + 1;
		NW = MakeShared<QuadTreeNode>(NWBox, NextDepth);
		NE = MakeShared<QuadTreeNode>(NEBox, NextDepth);
		SW = MakeShared<QuadTreeNode>(SWBox, NextDepth);
		SE = MakeShared<QuadTreeNode>(SEBox, NextDepth);

		bIsSubdivided = true;
	}

	FBox2D Bound;
	int32 Depth;

	TArray<QuadTreeNodeElement> Items;
	bool bIsSubdivided = false;

	TSharedPtr<QuadTreeNode> NW;
	TSharedPtr<QuadTreeNode> NE;
	TSharedPtr<QuadTreeNode> SW;
	TSharedPtr<QuadTreeNode> SE;
};

//SW(South west) 방향이 0,0 부터 시작하는 쿼드 트리.
class QuadTree
{
public:
	QuadTree()
	{
		MaximumDepth = 10;
		BoundaryWH = FVector2D(pow(2, MaximumDepth), pow(2, MaximumDepth));
		InnerBoundary = FBox2D(FVector2D(0, 0), BoundaryWH);
	}
public:
	int32 MaximumDepth;
	FBox2D WorldBoundary;

protected:
	FBox2D InnerBoundary;
	FVector2D BoundaryWH;
	TSharedPtr<QuadTreeNode> Root;


public:
	void AddElement(TWeakObjectPtr<UStaticMeshComponent> InComponent, const FVector& InPosition)
	{
		if (Root == nullptr)
		{
			Root = MakeShared<QuadTreeNode>(WorldBoundary, 0);
		}

		
		FVector2D QuadTreeLoc;
		QuadTreeLoc.X = InPosition.X;
		QuadTreeLoc.Y = InPosition.Y;
		QuadTreeNodeElement QTElement;
		QTElement.Component = InComponent;
		QTElement.QTPosition = QuadTreeLoc;
		Root->InsertData(QTElement, MaximumDepth);
	}

	void GetNearestElements(const FVector& SearchPos, TArray<QuadTreeNodeElement>& OutArray)
	{
		FVector2D QuadTreeLoc;
		QuadTreeLoc.X = SearchPos.X;
		QuadTreeLoc.Y = SearchPos.Y;
		Root->GetNearestElements(OutArray, QuadTreeLoc, MaximumDepth);
	}

	void GetAllSubdivisions(TArray<FBox2D>& OutIterations)
	{
		Root->GetBoundaries(OutIterations,MaximumDepth);
	}

	/// <summary>
	/// (0,0,10000,10000) 이 -> (0,0,1024,1024) 로 바뀌거나
	/// (-6000,-3000,7000,10000) -> (0,0,1024,1024) 로 바뀌어야함.
	/// (500,3500,13500,165000) ->
	/// 
	/// </summary>
	/// <param name="InUnrealPos"></param>
	/// <returns></returns>
	const FVector2D UnrealWorldToQuadTreeLocation(const FVector& InUnrealPos) const
	{
		const FVector2D Max = WorldBoundary.Max;
		const FVector2D Min = WorldBoundary.Min;

		const FVector2D Cur = FVector2D(InUnrealPos.X, InUnrealPos.Y);

		return ((Cur - Min) / (Max - Min)) * BoundaryWH;
	}

	const FVector QuadTreeLocationToUnrealWorldXY(const FVector2D& InQuadTreePos) const
	{
		const FVector2D Max = WorldBoundary.Max;
		const FVector2D Min = WorldBoundary.Min;

		const FVector2D CurUnreal = (InQuadTreePos / BoundaryWH) * (Max - Min) + Min;

		FVector Result = FVector();
		Result.X = CurUnreal.X;
		Result.Y = CurUnreal.Y;
		Result.Z = 100;//Only 2D, Because it doesn't save z position.

		return Result;
	}
};

UCLASS()
class SPATIALPARTITIONING_API ASPQuadTreeActor : public ASPBaseActor
{
	GENERATED_BODY()
	
protected:
	virtual void InitProperties() override;
	virtual void InitStaticMeshComponents() override;
	virtual void UpdatePartitioningState() override;
	virtual void DrawDebugObjects() override;

protected:
	UPROPERTY(EditAnywhere)
	int32 MaximumDepth = 5;

	TSharedPtr<QuadTree> QT = nullptr;
};
